...
{
	m_Camera.SetPosition({-70, 0, 30}, {0, 0, 0});
}

void PerspectiveCameraController::OnUpdate(float deltaMilisceonds)
{
	if (Input::IsKeyPressed(GLFW_KEY_W))
		m_Camera.Translate(m_CameraSpeed * m_Camera.GetFront() * deltaMilisceonds);
	if (Input::IsKeyPressed(GLFW_KEY_S))
		m_Camera.Translate(-m_CameraSpeed * m_Camera.GetFront() * deltaMilisceonds);
	if (Input::IsKeyPressed(GLFW_KEY_A))
		m_Camera.Translate(-glm::normalize(glm::cross(m_Camera.GetFront(), m_Camera.GetUp())) *
						   m_CameraSpeed * deltaMilisceonds);
